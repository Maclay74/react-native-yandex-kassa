...
// Deprecated since 5.4, replaced with THMConnectionTimeout
...
#define THMLocationServicesOnMainThread THM_NAME_PASTE(THM_PREFIX_NAME, THMLocationServicesOnMainThread)
...
#define THMSubjectPublicKeyInfoHashes   THM_NAME_PASTE(THM_PREFIX_NAME, THMSubjectPublicKeyInfoHashes)
...
#define THMDisableNonFatalLog           THM_NAME_PASTE(THM_PREFIX_NAME, THMDisableNonFatalLog)
...
#define THMMethodSwizzling              THM_NAME_PASTE(THM_PREFIX_NAME, THMMethodSwizzling)
#endif
...
#endif
#define THMConnectionTimeout            THM_NAME_PASTE(THM_PREFIX_NAME, THMConnectionTimeout)
#define THMRetryCount                   THM_NAME_PASTE(THM_PREFIX_NAME, THMRetryCount)
#define THMProfileTimeout               THM_NAME_PASTE(THM_PREFIX_NAME, THMProfileTimeout)
 ...
 * @discussion Valid at init time to set a key for profiling (different than session query API key)
...
/*!
 * @const THMTimeout, deprecated from 5.4, replaced with THMConnectionTimeout
 ...
 */
EXTERN NSString *const THMTimeout __attribute__((deprecated));
...
/*!
 * @const THMLocationServicesOnMainThread
 * @abstract NSDictionary key to specify if location services should be enabled on the main thread.
 * @discussion Valid at init time to enable location services on the main thread. Note that this should
 * be used in combination with THMLocationServices or THMLocationServicesWithPrompt.
 ...
 * Default value is \@NO (note use of NSNumber to store BOOL)
 */
EXTERN NSString *const THMLocationServicesOnMainThread;
 ...
 */
EXTERN NSString *const THMOptions __attribute__((deprecated("Please use THMEnableOptions or THMDisableOptions")));
 ...
 * @abstract NSDictionary key for setting a SHA1/SHA256 of certificate of Enhanced Profiling server.
 * @discussion Valid at init time for setting a SHA1/SHA256 of certificate of Enhanced Profiling server
 */
EXTERN NSString *const THMCertificateHashes;

/*!
 * @const THMSubjectPublicKeyInfoHashes
 * @abstract NSDictionary key for setting a SHA256 of SubjectPublicKeyInfo of Enhanced Profiling server.
 * @discussion Valid at init time for setting a SHA256 of SubjectPublicKeyInfo of Enhanced Profiling server
 */
EXTERN NSString *const THMSubjectPublicKeyInfoHashes;
...
/*!
 * @const THMDisableNonFatalLog
 * @abstract NSDictionary key for disabling non-fatal SDK logs.
 * @discussion Valid at init time for fine grained control over printing non-fatal logs.
 */
EXTERN NSString *const THMDisableNonFatalLog;
 ...
 * automatically. Should be @NO in case integrity protecion tools are used in the
 * final application. Note that by disabling method swizzling the application is
 * reponsible for passing the push token to SDK.
 */
EXTERN NSString *const THMMethodSwizzling;
#endif
...
/*!
 * @const THMConnectionTimeout, New option with clear meaning for THMTimeout
 * @abstract NSDictionary key for specifying the individual network timeout.
 * @discussion Valid at init time to set the individual network timeout, defaults to 10s.
 * Default is \@10 (note use of NSNumber to store int)
 */
EXTERN NSString *const THMConnectionTimeout;

/*!
 * @const THMRetryCount
 * @abstract NSDictionary key for specifying the individual network failure retrying count.
 * @discussion Valid at init time to set the retry count, defaults to 0.
 * Default is 0, no retrying (note use of NSNumber to store int)
 */
EXTERN NSString *const THMRetryCount;

/*!
 * @const THMProfileTimeout
 ...
 */
EXTERN NSString *const THMProfileTimeout;
